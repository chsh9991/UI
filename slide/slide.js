...
*/
  $(document).ready(function() {
    ...
    // 무한 슬라이드를 위해 start, end 슬라이드 복사하기
    const $startSlide = $slideItems.first();
    const $endSlide = $slideItems.last().clone();
    const $startElem = $startSlide.clone();
    const $endElem = $endSlide.clone();

    $endElem.addClass($endSlide.attr("class"));
    $endElem.html($endSlide.html());

    $startElem.addClass($startSlide.attr("class"));
    $startElem.html($startSlide.html());
    ...
    function nextMove() {
      ...
      if (currSlide <= maxSlide) {
        ...
        currSlide = 0;
        let offset = slideWidth * currSlide;
        $slideItems.css("transition", "0s");
        $slideItems.css("left", -offset);
        currSlide++;
        offset = slideWidth * currSlide;
        // 각 슬라이드 아이템의 left에 offset 적용
        setTimeout(function() {
          $slideItems.css("transition", "0.15s");
          $slideItems.css("left", -offset);
        }, 0);
        // 슬라이드 이동 시 현재 활성화된 pagination 변경
        $paginationItems.removeClass("active");
    ...
    for (let i = 0; i < maxSlide; i++) {
      // 각 페이지네이션마다 클릭 이벤트 추가하기
      $paginationItems.eq(i).click(function() {
        // 클릭한 페이지네이션에 따라 현재 슬라이드 변경해주기(currSlide는 시작 위치가 1이기 때문에 + 1)
        currSlide = i + 1;
        // 슬라이드를 이동시키기 위한 offset 계산
        const offset = slideWidth * currSlide;
    ...
    $slide.on("touchstart", function(e) {
      startPoint = e.originalEvent.touches[0].pageX; // 터치가 시작되는 위치 저장
    });

    $slide.on("touchend", function(e) {
      endPoint = e.originalEvent.changedTouches[0].pageX; // 터치가 끝나는 위치 저장
      if (startPoint < endPoint) {
        // 오른쪽으로 스와이프 된 경우
        prevMove();
      } else if (startPoint > endPoint) {
